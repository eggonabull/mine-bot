...
function inject(bot) {
  ...
  var actions = new (function ActionList() {
    ...
    this.goto = function (username, args) {
      ...
      var scaledOffset = desiredOffset
        .scaled((1 / offsetDistance) * 100)
        .floored();
      logSay("scaledOffset: " + scaledOffset.toString());
      var localTarget = bot.entity.position.plus(scaledOffset);
      var highestBlockForColumn = findHighestBlockInColumn(
        localTarget.x,
        localTarget.z,
      );
        ...
        if (err) {
          ...
          if (bot.entity.position.equals(botPosition)) {
            ...
            bot.setTimeout(function () {
              actions.goto(username, args);
            }, 1000);
          }
        } else {
          logSay(
            "made it to " + bot.entity.position.x + " " + bot.entity.position.z,
          );
          bot.setTimeout(function () {
            actions.goto(username, args);
          }, 1000);
