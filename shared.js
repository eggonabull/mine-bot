...
export async function pickUpItems() {
  ...
  for (const entity_id in bot.entities) {
    ...
    if (entity.type === "hostile" || entity.type === "player") {
      continue;
    }
...
export function has_thorns(entity) {
...
export function getDistances(vec) {
  const bot = g.bot;
...
export async function drop(filter) {
  const bot = g.bot;
...
export async function equip_by_id(item_id) {
  const bot = g.bot;
  const item = bot.inventory.findInventoryItem(item_id);
  if (item) {
    await bot.equip(item, "hand");
  }
}

export async function equip_by_name(item_name) {
  const bot = g.bot;
